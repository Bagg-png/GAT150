...
namespace ag{
	...
	{
		// Prevents the sequence from replaying over itself
	...
	bool SpriteAnimationComponent::Read(const rapidjson::Value& value)
	{
		...
		{
			endFrame = (numFramesX * numFramesY) - 1;
		}
		frame = startFrame;
		...
		if(value.HasMember("sequences") && value["sequences"].IsArray())
		{
			...
			std::string defaultSequnce;
			JSON_READ(value, defaultSequnce);
			SetSequence(defaultSequnce);
		}
