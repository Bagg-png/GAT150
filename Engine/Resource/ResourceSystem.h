...
namespace ag {
	...
	public:
		...
		void Add(const std::string& name, std::shared_ptr<ag::Resource> resource);
	private:
	...
	inline void ResourceSystem::Add(const std::string& name, std::shared_ptr<ag::Resource> resource) {
		resources[string_tolower(name)] = resource;
	}
}
